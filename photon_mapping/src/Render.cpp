...
void Render::renderizarPixel(
	...
	RGBQUAD color_directo;
	color_directo.rgbRed = 255.0f * clamp(raytracer.rt_directo.x, 0.0f, 1.0f);
	color_directo.rgbGreen = 255.0f * clamp(raytracer.rt_directo.y, 0.0f, 1.0f);
	color_directo.rgbBlue = 255.0f * clamp(raytracer.rt_directo.z, 0.0f, 1.0f);
	...
	FreeImage_SetPixelColor(bitmap_directa, x, height - y - 1, &color_directo);
}
